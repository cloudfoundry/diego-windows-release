...
function ssh_remote {
    if [ "x${JUMP_MACHINE_IP}" != "x" ]; then
        local proxyCommand="ssh -A -l ec2-user -x ${JUMP_MACHINE_IP} -W %h:22"
        echo $(ssh ${ssh_opts} ci@${MACHINE_IP} -o "ProxyCommand=${proxyCommand}" $1)
    else
        echo $(ssh ${ssh_opts} ci@${MACHINE_IP} $1)
    fi
}
